...
def upgrade() -> None:
    ...
    if "ai_logical_requests" not in inspector.get_table_names():
        ...
        if "started_at" not in logical_columns:
            ...
            op.alter_column("ai_logical_requests","started_at",nullable=False)
    ...
    if "fk_ai_runs_logical_request" not in foreign_keys:
        op.create_foreign_key("fk_ai_runs_logical_request","ai_runs","ai_logical_requests",
    ...
    for name,condition in (("ck_ai_runs_input_tokens","input_tokens IS NULL OR input_tokens >= 0"),
        ...
        if name not in checks: op.create_check_constraint(name,"ai_runs",condition)


def downgrade() -> None:
    op.drop_constraint("fk_ai_runs_logical_request","ai_runs",type_="foreignkey",if_exists=True)
    ...
    for name in ("ck_ai_runs_retry_count","ck_ai_runs_estimated_cost","ck_ai_runs_output_tokens","ck_ai_runs_input_tokens"):
        op.drop_constraint(name,"ai_runs",type_="check",if_exists=True)
